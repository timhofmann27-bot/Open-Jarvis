...
def _get_client() -> genai.Client:
    """Create a fresh Gemini client (new google-genai SDK)."""
    return genai.Client(api_key=_get_api_key())


def _normalize_contents(contents):
    """Convert old-style dict parts to new-style types.Part objects."""
    if not isinstance(contents, list):
        return contents
    new_contents = []
    for c in contents:
        if isinstance(c, dict) and "mime_type" in c and "data" in c:
            new_contents.append(types.Part.from_bytes(data=c["data"], mime_type=c["mime_type"]))
        else:
...
def _generate(contents) -> str:
    """Generate content using Gemini and return the text.

    contents can be:
    - str: text prompt
    - list: list of parts (text, image, audio, etc.)
    """
    client = _get_client()
